...
if [ $test_name == "ncols100" ]; then
  ...
  cmd4="bin/run-query --num-objs ${total_num_objs} --pool ${poolname} --wthreads ${worker_threads} --qdepth ${queue_depth}  --transform-db --transform-format-type ${format} --data-schema ${DATA_SCHEMA_NCOLS100} --project-cols ${PROJECT_COLS_NCOLS100}"
else
  echo "using lineitem schema...."
  cmd4="bin/run-query --num-objs ${total_num_objs} --pool ${poolname} --wthreads ${worker_threads} --qdepth ${queue_depth}  --transform-db --transform-format-type ${format} --data-schema ${DATA_SCHEMA_LINEITEM} --project-cols ${PROJECT_COLS_LINEITEM}"
fi
echo "cmd4=$cmd4"
eval "$cmd4"
sleep 10;
