...
nruns=$4
cls=$5
echo "usage: ./qb.sh <pool> <nobjs> <nthreads> <nruns> [--use-cls]"
echo "--pool=$pool"
echo "--num-objs=$nobjs"
...
do
    ...
    t2=`date --utc "+%s.%N"`
    res=0$(echo "$t2 - $t1"|bc);
    echo "qb selectivity=0.1% pool=$pool nthreads=$nthreads cls=$cls run$i=$res"
done
#
...
do
    ...
    t2=`date --utc "+%s.%N"`
    res=0$(echo "$t2 - $t1"|bc);
    echo "qb selectivity=1% pool=$pool nthreads=$nthreads cls=$cls run$i=$res"
done
#
...
do
    ...
    t2=`date --utc "+%s.%N"`
    res=0$(echo "$t2 - $t1"|bc);
    echo "qb selectivity=100% pool=$pool nthreads=$nthreads cls=$cls run$i=$res"
done
